...
{
    public class SetWorkedByOnAddToQueueItem_PostUpdate : IPlugin
    {
        public void Execute(IServiceProvider serviceProvider)
        {
            ...
            if (exeContext.PrimaryEntityName == QueueItemEntityAttributeNames.EntityLogicalName)
            {
                try
                {
                    ...
                    if (exeContext.Stage == (int)PluginStages.PostOperation && exeContext.MessageName == PluginHelperStrigs.AddToQueueMsgName)
                    {
                        presenterQueueItem.SetWorkedBy_AddToQueueItem(serviceFactory, exeContext, srvTracing);
